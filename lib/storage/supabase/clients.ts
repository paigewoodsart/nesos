...
export async function sbSaveClient(userEmail: string, client: Client): Promise<void> {
  try {
    ...
    await supabase.from("clients").upsert({ id: client.id, user_email: userEmail, name: client.name, color: client.color, notes: client.notes ?? null, created_at: client.createdAt });
  }
}
...
export async function sbSaveClientTask(userEmail: string, task: ClientTask): Promise<void> {
  try {
    ...
    await supabase.from("client_tasks").upsert({
      id: task.id, user_email: userEmail, client_id: task.clientId, text: task.text,
...
export async function sbSaveSession(userEmail: string, session: ClientSession): Promise<void> {
  try {
    ...
    await supabase.from("client_sessions").upsert({
      id: session.id, user_email: userEmail, client_id: session.clientId, week_id: session.weekId,
      day_index: session.dayIndex, start_minute: session.startMinute, end_minute: session.endMinute,
      actual_minutes: session.actualMinutes, notes: session.notes, date: session.date, created_at: session.createdAt,
    });
  }
}
...
function rowToClientTask(r: Record<string, unknown>): ClientTask {
  return {
    id: r.id as string, clientId: r.client_id as string, text: r.text as string,
    done: r.done as boolean, doneAt: r.done_at as number | null, dueDate: r.due_date as string | null,
    archived: r.archived as boolean, archivedAt: r.archived_at as number | null, createdAt: r.created_at as number,
  };
}

function rowToSession(r: Record<string, unknown>): ClientSession {
  return {
    id: r.id as string, clientId: r.client_id as string, weekId: r.week_id as string,
    dayIndex: r.day_index as number, startMinute: r.start_minute as number, endMinute: r.end_minute as number,
    actualMinutes: r.actual_minutes as number | null, notes: r.notes as string,
    date: r.date as string, createdAt: r.created_at as number,
  };
}
